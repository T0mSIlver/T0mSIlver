...
- **[mlx-audio-swift](https://github.com/Blaizzy/mlx-audio-swift/pulls?q=is%3Apr+author%3AT0mSIlver)** — contributor to the Voxtral realtime streaming path: incremental mel/conv front end (O(N²) → O(N) per utterance), fp16 dtype fixes, Metal buffer-pool policy, quantized tied-embedding loading. Streaming went from ~1.9× slower than realtime to a flat 0.76 RTF. `Swift`
...
- **[voxmlx](https://github.com/T0mSIlver/voxmlx)** — realtime Voxtral transcription in MLX; my fork adds an OpenAI-Realtime-compatible WebSocket server and Metal memory caps. 4-bit conversions on [Hugging Face](https://huggingface.co/T0mSIlver), including a [variant with a quantized tied LM head](https://huggingface.co/T0mSIlver/Voxtral-Mini-4B-Realtime-2602-4bit-qhead) that cuts the 768 MiB-per-decoded-token head read ~4×. `Python`
