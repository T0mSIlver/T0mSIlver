...
- **[skills](https://github.com/T0mSIlver/skills)** — Agent Skills that let a coding agent drive another coding CLI without losing control of the main checkout: a read-only reviewer and an edit-capable worker for `claude`, `codex` and `opencode`, a live codex pane driven over [herdr](https://herdr.dev), and persistent per-repo Remote Control servers under systemd. Installs with `npx skills add` or as a Claude Code plugin marketplace; vendored third-party skills stay pristine, and a daily workflow opens a PR when upstream moves. `Shell`
